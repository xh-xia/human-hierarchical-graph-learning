...
def main():
    ...
    16: _e427_live3_99subs | ver = "26.427" | actually 99+99 (2 batches of the same stim & table)
    """
    which_db = 16  # which database we want to parse
    check_prev_ids = True  # whether we check previously stored assignids_dict
    show_workerid = True
    ver = "26.427"
    cwd = set_dir427()
    folder_path = "/input/database/"
    csvs = ["exp", "participants", "walkdata"]
    df_dict = dict()
    ...
    cwd += folder_path + f"{which_db}/"
    kwargs = dict(db_ver=[which_db, ver], live_only=True, complete=False, threshold=None)
    kwargs.update(dict(cwd=cwd, check_prev_ids=check_prev_ids))
    kwargs.update(dict(show_workerid=show_workerid, assignids_set=get_assignids_set(cwd)))
    get_subjects_data(df_dict, **kwargs)


def get_assignids_set(cwd):
    assignids_set = set()
    with open(cwd + "assignmentids.json") as f:
        data = json.load(f)
    for d in data:
        assignids_set.update(d.values())
    return assignids_set


def get_subjects_data(df_dict, db_ver, live_only=True, complete=True, show_workerid=True,
                      assignids_set=None, threshold=None, cwd="", check_prev_ids=False):
    """
    Args
    ----
    ...
    assignids_set (set):
        assignmentid from HITs generated by copy pasting results from aws-shell:
        (mac:)
        mturk list-assignments-for-hit --hit-id XXX --query "Assignments[].{AssignmentId: AssignmentId}"
        in this sense this is the groudtruth,
        but we will print both those from this and those not in this just to make sure
    cwd (str):
        directory that we will read/write assignids_dict from/to
    check_prev_ids (bool):
        only concerns print results: whether we exclude those appears in previous assignids_dict
        however, it does determine whether we save the current assignids_dict (save if False)


    Intermediary
    ------------
    assignids_dict (dict): k:v -> assignmentid:list
    list = [workerid,n_back+walk-390 (390 is n-back total)]
        if assignmentid exists, check if the new val is larger, if so replace
        this variable is to aid in judging whether we should pay the subjects
        also multiple different workerids can have same assignmentid,
        and we pay by assignmentid. That's what this dict is for
    rmc (set): rmc = rare multiple completions; f"{workerid}:{assignid}"
        in which assignid that had full completion has multiple workerids
        this set includes all but the first workerid
    ...
    df_participants = df_participants.astype({'codeversion': 'str'})  # float64 -> str
    ...
    for UID in df.index:  # UID = workerid:assignmentid; loop through each participant
        tempdf = df_dict["exp"].loc[df_dict["exp"]["uniqueId"] == UID]
        if tempdf.empty:
            raise Exception(f"couldn't find UID={UID} in exp.db.")
        if len(tempdf.index) > 1:
            raise Exception(f"UID={UID} has more than 1 row in exp.db.")
    ...
    kwargs = dict(show_workerid=show_workerid, verbose=False, threshold=threshold, cwd=cwd)
    kwargs.update(dict(assignids_set=assignids_set, check_prev_ids=check_prev_ids))
    rmc = print_progresses(data_dict, **kwargs)
    # rmc = set()

    if complete:  # remove subjects who didn't complete the whole experiment AND who completed > once
        # use False since occasionally df_task is complete but not df_n_back
        # and we care mostly about df_task
        for k in data_dict:
            if (data_dict[k]["df_n_back"] is None) or (data_dict[k]["df_task"] is None) or k in rmc:
                del data_dict[k]
        ...
        n_back_all = pd.concat(
            [data_dict[k]["df_n_back"] for k in data_dict if (data_dict[k]["df_n_back"] is not None and k not in rmc)],
            ignore_index=True,
        )
        task_all = pd.concat(
            [data_dict[k]["df_task"] for k in data_dict if (data_dict[k]["df_task"] is not None and k not in rmc)],
            ignore_index=True,
        )
...
def print_progress(assignmentid, bonus_info, progress, sum_progress=None):
    if sum_progress is None:
        if isinstance(bonus_info, str):
            ...
            if isnan(bonus_info):
                print(f'assignmentid={assignmentid}: N/A')
                print(f'n-back progress: {progress["n_back"]} | walk progress: {progress["walk"]}')
        else:
            raise ValueError(f"bonus_info type={type(bonus_info)} is unclear")
    else:
        print(f"'{assignmentid}' | sum_prog: {sum_progress}")


def print_progresses(data_dict, show_workerid=False, verbose=False, threshold=None,
                     assignids_set=None, check_prev_ids=False, cwd=""):
    if verbose:  # superannuated; should be removed
        for UID in data_dict:
            bonus_info, progress = data_dict[UID]["progress_tuple"]
            print_progress(UID.split(":")[1], bonus_info, progress)
    else:
        rmc = set()
        assignids_dict = dict()  # it has all assignmentids
        assignids_full = set()  # we will accept these assignment ids
        for UID in data_dict:
            bonus_info, progress = data_dict[UID]["progress_tuple"]
            n_nback = int(progress["n_back"].split("/")[0])
            n_walk = int(progress["walk"].split("/")[0])
            n_sum = n_nback + n_walk - 390
            workerid, assignid = UID.split(":")
            if assignid not in assignids_dict:
                assignids_dict[assignid] = [workerid, n_sum]
            else:
                if n_sum > assignids_dict[assignid][1]:
                    assignids_dict[assignid] = [workerid, n_sum]

            if isinstance(bonus_info, float):
                continue
            elif isinstance(bonus_info, str):
                if n_sum == 1500:  # full completion
                    if assignid not in assignids_full:
                        assignids_full.add(assignid)
                    else:
                        print(f"assignment={assignid} completed full task twice; workerid:")
                        print(f"last: {assignids_dict[assignid]}")
                        print(f"current: ['{workerid}', {n_sum}]")
                        rmc.add(f"{workerid}:{assignid}")

        if check_prev_ids:
            with open(cwd + 'assignids_dict.json', 'r') as f:
                prev_assignids_dict = json.load(f)
            # delete keys that show up in prev_assignids_dict
            for k in prev_assignids_dict:
                del assignids_dict[k]
        else:  # save assignids_dict for future use (i.e., when check_prev_ids == True)
            with open(cwd + 'assignids_dict.json', 'w') as f:
                json.dump(assignids_dict, f, sort_keys=True, indent=4)

        def temp_print(counter, x, assignids_dict, show_workerid):
            if show_workerid:
                print(f"'{assignids_dict[x][0]}:{x}'")
            else:
                print(f"'{x}'")
            return counter + 1
        ...
        for i in [0, 1]:
            if i == 0:
                ...
                assignids_set_current = assignids_set
            else:
                print(">>>extra assignments<<<")
                assignids_set_current = set(assignids_dict.keys()) - assignids_set
            print(f"Full completion: 390/390 & 1500/1500")
            counter = 0
            for x in assignids_set_current:
                if assignids_dict[x][1] == 1500:
                    counter = temp_print(counter, x, assignids_dict, show_workerid)
            print(f"total number of subjects: {counter}")
            print("", end="\n\n")


            if threshold is not None:
                print(f"Bare completion: 390/390 & >= {threshold}/1500")
                counter = 0
                for x in assignids_set_current:
                    if threshold <= assignids_dict[x][1] < 1500:
                        counter = temp_print(counter, x, assignids_dict, show_workerid)
                print(f"total number of subjects: {counter}")
                print("", end="\n\n")

                print(f"Rejected completion: < 390/390 | < {threshold}/1500")
                counter = 0
                for x in assignids_set_current:
                    if assignids_dict[x][1] < threshold:
                        counter = temp_print(counter, x, assignids_dict, show_workerid)
                print(f"total number of subjects: {counter}")
                print("", end="\n\n")
            else:
                print(f"Satisfactory completion: 390/390 & >= 750/1500")
                counter = 0
                for x in assignids_set_current:
                    if 750 <= assignids_dict[x][1] < 1500:
                        counter = temp_print(counter, x, assignids_dict, show_workerid)
                print(f"total number of subjects: {counter}")
                print("", end="\n\n")

                print(f"Unsatisfactory completion: < 390/390 | < 750/1500")
                counter = 0
                to_sort = dict()
                for x in assignids_set_current:
                    if assignids_dict[x][1] < 750:
                        counter += 1
                        # bonus_info, progress = data_dict[UID]["progress_tuple"]
                        if show_workerid:
                            to_sort[f"{assignids_dict[x][0]}:{x}"] = assignids_dict[x][1]
                        else:
                            to_sort[x] = assignids_dict[x][1]
                to_sort = dict(sorted(to_sort.items(), key=lambda x: x[1], reverse=True))
                for k, v in to_sort.items():
                    print_progress(k, None, None, v)
                print(f"total number of subjects: {counter}")
                print("", end="\n\n")
...
def get_completion(questiondata):
    ...
    progress = {"n_back": "0/390", "walk": "0/1500"}
...
def get_csv_name(cwd, csvs, which_test=0):
    """
    Args:
        csvs (list): e.g., ["exp", "participants", "walkdata"]
...
def save_df_to_csv(df, folder_name, fname, show_df=False, index=True):
    """
    Args:
    -----
    ...
    index (bool): whether we write index as first column
    """
    if show_df:
        print(f"DEBUG df:\n{df}")
    ...
    dir_ += fname + ".csv"
    df.to_csv(dir_, sep=",", quotechar='"', index=index)  # save to csv file
