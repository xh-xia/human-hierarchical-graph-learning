...
def main_Sierpinski427():
    ...
    p["colors"] = colors_selector(str="7-class Greens")
    p["beta_arr"] = np.geomspace(4e-4, 2e1, 400)  # for analytical curve only
    hierDict = dict()
    # we need to have below 3 parameters in params.json when initializing simulation
    p["sub_fo_name"] = "emp_unbinned_beta_10kperbeta"
    p["key_class"] = "emp_unbinned_beta_10kperbeta"
    p["n_agents"] = 10000
    if FIGNUM == 2:
        hierDict['reg_p'] = [[3],[3, 4, 5],[3]]  # paper Fig.2
        p["mode427"] = 0  # needed for "reg_"
    elif FIGNUM == 3:
        hierDict['reg_n'] = [[3], [3], [3, 4, 5]]  # paper Fig.3
        p["mode427"] = 0  # needed for "reg_"
    elif FIGNUM == 4:  # also generate Fig.7 and Fig.8
        hierDict["emp"] = [[3],[3],[3]]  # experiment only; paper Fig.5
        p["sub_fo_name"] = "emp_unbinned_beta_10kperbeta"
        p["key_class"] = "emp_unbinned_beta_10kperbeta"
        p["n_agents"] = 10000
        p["mode427"] = 1
    elif FIGNUM == 5:
        p["sub_fo_name"] = "emp_unbinned_beta_100perbeta_7500"
        p["key_class"] = "emp_unbinned_beta_100perbeta_7500"
        p["n_agents"] = 100
        hierDict["alt"] = [[3],[3],[3]]  # exp setup but with longer walk lengths; paper Fig.6
        p["mode427"] = 1
    elif FIGNUM == 6:
        hierDict['reg_p'] = [[0,1,2,3],[3, 4, 5],[3]]  # supplement Fig.1
        p["mode427"] = 0  # needed for "reg_"
    elif FIGNUM == 7:
        hierDict['reg_n'] = [[0,1,2,3],[3],[3, 4, 5]]  # supplement Fig.2
        p["mode427"] = 0  # needed for "reg_"

    # key_class needed to be run in sim427
    # otherwise error in load_CCS_stat(), which loads results generated from sim427
    # p["key_class"] = "max_beta"
    p = make_sim_params(p, binned=False)
    p.update(get_params(fname="input\\params_CCS427", default_dir=False))  # get parameters
    ...
    kw_loop.update(dict(mode427=p["mode427"], fig_num=FIGNUM))
...
def plot_main(beta_class, sub_fo_name, CCS_type, key_class, n_agents, hierDict,
              beta_arr, err_type, raw_method, CCS_plot_type, colors, dpi, mode427, fig_num):
    ...
    ----
    - mode427 (int):
    0 analytical; 1 analytical+simulated
    ...
    for key in hierDict.keys():
        ...
        kw_plot = dict(CCS_stat=CCS_stat, err_type=err_type, CCS_type=CCS_type, fig_num=fig_num)
        if key.startswith("reg_"):
            ...
            regCCS = 2
        if mode427 == 0:  # override key diff
            regCCS = 6
        if IS_SUPP:
            regCCS = 1  # override
        ...
        if regCCS in [0, 1]:
            ...
            kw_plot.update(dict(spl=slice(0, None)))  # doesn't matter, will be overridden anyway
            plot_Graph_CCS(DD, beta_arr, key, **kw_plot)
        elif regCCS in [4, 5]:
...
def plot_side(tup, beta_arr):
    ...
    - beta_arr (nparr): for analytical curve, 400 pts
...
def FX_MC427(noise, params, res=1000, n_samp_grid=None, fx=1, all_beta=True):
    ...
    1) was done in getting the noise
    2) MC simulation to draw from noise
    for each beta in noise:
        for each sample size n_samp in n_samp_grid:
...
def plot_Graph_CCS(
    ...
    CCS_plot_type="CCS", colors=None, dpi=None, regCCS=0, sub_folder_name="", mode427=1,
    fig_num=2
):
    ...
    if regCCS == 2:  # force spl to be a len-2 list [0, -1]
        ...
        spl = [0, 2, 4]  # 3 walk sizes: 1500, 4500, 7500
    if CCS_stat is not None and regCCS not in [2, 4, 5, 7]:
    ...
    kw_gs = {"nrows": len(height_ratios), "ncols": len(width_ratios)}
    kw_gs.update({"height_ratios": height_ratios, "width_ratios": width_ratios})
    gs = GridSpec(**kw_gs)
    ...
    if regCCS==1:  # assuming DD_keys has 12 entries
        for i in range(3):  # col
            for regType in [0, 1, 2, 3]:  # row
                ...
                kw2.update(dict(show_legend=(i == 2 and regType == 3)))
                ax_CCS(**kw2, **kw_gb)
                if varb:
        ...
        for i in range(3):  # col
            for row in spl:  # row
                ...
                kw2.update(dict(raw_method=raw_method, show_legend=i == 2, has_title=True))
                ax_CCS(**kw2, **kw_gb)
                if varb:
                    kw2.update(dict(ax=axes2[f"CCS_row{regType}"][i], varb=varb))
                    ax_CCS(**kw2, **kw_gb)
        ...
        for i in range(3):
            ...
            params = DD_keys[i]
            kw1 = dict(ax=axes["Graph"][i], axcb=axes["Colorbar"][i], fig=fig, dpi=dpi)
            kw1.update(dict(params=params, colors=colors))
            kw1.update(dict(nodeList=DD[params]["Sier"].nodeList, GTDict=DD[params]["GTDict"]))
            ax_Graph(**kw1)
            kw2 = dict(ax=axes["CCS"][i], x=beta_arr, params=params)
            kw2.update(dict(spl=spl_current, CCS_plot_type=CCS_plot_type))
            kw2.update(dict(CCS_arr=DD[params]["CCS_arr"], noise=CCS_stat))
            kw2.update(dict(key=key, err_type=err_type, CCS_type=CCS_type))
            kw2.update(dict(show_legend=i == 2, is_log=True, colors=colors, dpi=dpi, has_xlabel=lr))
            kw2.update(dict(raw_method=raw_method, has_title=True))
            ax_CCS(**kw2, **kw_gb)
            if varb:
                ...
                kw1.update(dict(ax=axes2["Graph"][i], axcb=axes2["Colorbar"][i], fig=fig2))
                ax_Graph(**kw1)
                kw2.update(dict(ax=axes2["CCS"][i], varb=varb))
                ax_CCS(**kw2, **kw_gb)
        if regCCS==2:  # two CCS rows: one with walk_length=1500, one with walk_length=7500
            ...
            for i in range(3):
                ...
                params = DD_keys[i]
                kw2.update(dict(ax=axes["CCS"][i+3]))
                kw2.update(dict(CCS_arr=DD[params]["CCS_arr"], params=params))
                kw2.update(dict(spl=spl_current, show_legend=i == 2, has_xlabel=True))
                ax_CCS(**kw2, **kw_gb)
        ...
        for i in range(DD_keys[0][2]):  # col: all CCS level + per CCS level
            for row in range(5):  # row
                ...
                kw2.update(dict(params=params, key=key))
                kw2.update(dict(CCS_arr=DD[params]["CCS_arr"], noise=CCS_stat))
                kw2.update(dict(emp_lv=temp_emp_lv))
                kw2.update(dict(err_type=err_type, dpi=dpi, CCS_type=CCS_type))
                ...
                ax_CCS(**kw2, **kw_gb)
    elif regCCS in [5, 7]:  # DD_keys has 1 entry: (3, 3, 3)
        if regCCS == 5:
            spl_current = [spl] * 3
            temp_n_walk = [None] * 3
            ...
            spl_current = spl
            temp_n_agents = [None] * 3
            temp_n_walk = [round(CCS_stat["mean"][DD_keys[0]][spl[i], 2, 0]) for i in range(3)]
        else:
            raise NotImplementedError(f"<regCCS>={regCCS} has to be 5 or 7")
        for i in range(3):  # col: each n_per_bin OR each walk length
            for row in range(n_level):  # row
                temp = gs[row * 4 : row * 4 + 3, i * 3 + 1]
                axes[f"CCS_row{row}"][i] = fig.add_subplot(temp)
                params = DD_keys[0]
                temp_emp_lv = row + 1
                ...
                kw2.update(dict(emp_lv=temp_emp_lv))
                kw2.update(dict(err_type=err_type, dpi=dpi, CCS_type=CCS_type))
                kw2.update(dict(is_log=True, colors=colors, has_xlabel=row==(DD_keys[0][2] - 2), has_title=row==0))
                kw2.update(dict(spl=spl_current[i], raw_method=raw_method, show_legend=True))
                kw2.update(dict(n_agents=temp_n_agents[i], n_walk=temp_n_walk[i]))
                ax_CCS(**kw2, **kw_gb)
    else:  # DD_keys has 1 entry: (3, 3, 3)
        for i in range((n_level)):  # col
            temp = gs[0 : 3, i * 3 + 1]
            ...
            ax_CCS(**kw2, **kw_gb)
    # panel label list
    regCCS2row = {0:2, 1:4, 2:3, 3:5, 4:5, 5:n_level, 6:2, 7:n_level}  # k:v -> regCCS:num_of_rows
    # except for 4, which is regCCS:num_of_cols
    if regCCS in regCCS2row.keys():
        text_labels = [f"({chr(ord('a') + i)})" for i in range(regCCS2row[regCCS])]
    else:
        raise NotImplementedError(f"<regCCS>={regCCS} is not implemented")
...
def plot_corr(params, noise, colors, sub_folder_name, res=1000, res_grid=1000, dpi=None, truncated=False):
    ...
    for i in range(n_level - 1):  # do it for each two CCS lvs
        ...
        sty1.update(dict(color="red", label=f"Empirical $r_s$(lv{i+1},lv{i+2})"))  # ECCS2 sample size = 87; r = -0.468
        axes[i].scatter([80], [-0.468], s=6, **sty1)
        ...
        axes[i].set_title("Cross CCS Level Spearman Correlation", fontsize=FS_MAIN)
        axes[i].set_xlim(xlim)
        axes[i].set_ylim(ylim)
        axes[i].set_yticks(yticks)
        axes[i].tick_params(axis="both", which="major", **TICK_PARAMS)
        axes[i].tick_params(axis="both", which="minor", **TICK_PARAMS_MINOR)
        axes[i].set_xlabel(r"Total Sample Size Across All $\beta$", fontsize=FS_LAB)
        axes[i].set_ylabel("Spearman Correlation", fontsize=FS_LAB)
    ...
    fname = f"Fig8.corr{temp_str}_param=({params[0]:d},{params[1]:d},{params[2]:d})"
    saveNclose427(fig, fname, dpi=dpi, sub_folder_name=sub_folder_name)
...
def plot_power(params, noise, colors, sub_folder_name, res=1000, pvals=None, dpi=None):
    ...
    n_samp_emp = [1, 3, 2, 10, 20, 24, 21, 6, 0, 0]  # hand coded
    bindices = [1, 3, 4, 5, 6, 7]  # idx of bins whose sample size > 2
    bbdry_nocenter = np.array([ [5.995e-04,1.668e-03],
                                [1.668e-03,4.642e-03],
                                [4.642e-03,1.292e-02],
                                [1.292e-02,3.594e-02],
                                [3.594e-02,1.000e-01],
                                ...
                                [2.783e-01,7.743e-01],
                                [7.743e-01,2.154e+00],
                                [2.154e+00,5.995e+00],  # this last two rows unused cuz n=0
                                [5.995e+00,1.668e+01]])
    if pvals is None:
        # pvals = [0.05, 0.01, 0.001]
        pvals = [0.05, 0.01]
    vs = [None] * len(pvals)
    fname = f"Fig7.powers=({params[0]:d},{params[1]:d},{params[2]:d})"
    prefix = "input\\npy_files\\"
    ...
    # plot the power
    if len(pvals) == 1:  # 2 row by 3 col; only show when emp samp size > 2, which is 6 bins
        fname += "emp_samp_geq2"
        ...
        for i in range(2):  # row loop
            for j in range(3):  # col loop
                ...
                axes[i][j].tick_params(axis="both", which="major", **TICK_PARAMS)
                axes[i][j].tick_params(axis="both", which="minor", **TICK_PARAMS_MINOR)

                bidx = bindices[i * 3 + j]
                str_title = f"[{bbdry_nocenter[bidx, 0]:.3e},{bbdry_nocenter[bidx, 1]:.3e}]"
                axes[i][j].set_title(f"β∈{str_title}", fontsize=FS_MAIN)
                if i == 1:  # 2nd row
                    axes[i][j].set_xlabel("Sample Size", fontsize=FS_LAB)
        ...
        for i in range(b):  # row loop
            for j in range(len(pvals)):  # col loop
                ...
                if xlim[0] > 0:
                    axes[i][j].plot(xlim, (0, 1), ":", color="red", zorder=0)  # emp samp size
...
def ax_CCS(ax, x, CCS_arr, params, key, CCS_plot_type="CCS", raw_method=None, emp_lv=None,
           ...
           is_log=True, colors=None, dpi=None, has_xlabel=True, has_title=True,
           n_agents=None, n_walk=None, fig_num=2):
    """
    Args
    ----
    - ax: axis object
    ...
    - spl (int): what index of sample to draw data from CCS_stat; -1 is the last sample; NOT list
    - n_agents (None or int): number of agents to be included
    - n_walk (None or int): walk length to be included
    ...
    if n_walk is not None:
        title += f", Walk Length of {n_walk:d}"
    ...
    def temp_b1(i):  # annotate CCS plot with maxima
        ...
        if noise is None:
            if fig_num == 2:
                kw_text.update(dict(xytext=(0.85 - i * 0.6, 0.90 - i * 0.2)))
            if fig_num == 3:
                if i < 2:
                    kw_text.update(dict(xytext=(0.85, 0.90 - i * 0.2)))
                else:
                    kw_text.update(dict(xytext=(0.6 - (i - 2) * 0.3, 0.15)))
            else:
                kw_text.update(dict(xytext=(0.85, 0.90 - i * 0.1)))
        else:
            kw_text.update(dict(xytext=(0.85, 0.75)))
...
def ax_Graph(ax, axcb, fig, params, nodeList, GTDict,
    ...
    if bA is not None:  # a singleton graph with transition prob edge weight coloring
        ...
        cbr.ax.tick_params(axis="both", which="major", **TICK_PARAMS)
        cbr.ax.tick_params(axis="both", which="minor", **TICK_PARAMS_MINOR)
        cbrLabel427(axcb, "transition probability", fontsize=FS_LAB)
        ...
        cbr.ax.tick_params(axis="both", which="major", **TICK_PARAMS)
        cbr.ax.tick_params(axis="both", which="minor", **TICK_PARAMS_MINOR)
        cbrLabel427(axcb, "Edge Level", fontsize=FS_LAB)
        for lv in all_levels:
            ...
            ax.plot(
                xcoords, ycoords, color=cmap(lv - 1), zorder=1, linewidth=3.1
            )  # lower int -> drawn on the canvas earlier; linewidth=1.5 (default)
