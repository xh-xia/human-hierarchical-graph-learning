...
class hierList:
    ...
    def Rotate(
        self, theta, pivot, i_=None
    ):  # coordinates rotation (only rotate if elements are coor)
        """RIGHT rotation
        ...
        if i_ is None:  # rotate all
            if isinstance(
                self.hlist[0], hierList
            ):  # check if any element of hlist is also hierList
                for i in range(self.length):
                ...
                for i in range(self.length):
                    self.hlist[i] = [
                        CO * (self.hlist[i][0] - pivot[0])
                        + SO * (self.hlist[i][1] - pivot[1])
                        + pivot[0],
                        -SO * (self.hlist[i][0] - pivot[0])
                        + CO * (self.hlist[i][1] - pivot[1])
                        + pivot[1],
                    ]
        else:  # rotate i_-th member only
            i = (self.index_start + i_) % self.length
            if isinstance(
                self.hlist[i], hierList
            ):  # check if that element of hlist is also hierList
...
class make_Sierpinski427:
    def __init__(
        self, p, n, x0=[0.0, 0.0], s0=1.0, c=1.0, regType=0
    ):  # constructor; n is vertex name/index
        """
        regType (int):
        ...
        self.c = (
            c  # constant offset value that adds to the previous radius to make the new shift length
        )
    ...
    def set_params(self):
        ...
        if self.p <= 3:
            ...
            self.c_arr = [
                self.c + (i ** 2 - 0.7 * i + 0.24) * 0.618 * (self.p - 4) ** 2
        ...
        self.radii = [
            2 ** j * self.s0 + sum([2 ** (j - i) * self.c_arr[i] for i in range(1, j + 1)])
            for j in range(0, self.n)
        ]
        self.shift_len = [self.s0] + [
            self.radii[j - 1] + self.c_arr[j] for j in range(1, self.n)
        ]  # never used radii[n-1]
        """ ↓ calculates pivot at all stages j∈[0,...,n-1] ↓ """
        p0 = (
            self.x0[0] + self.shift_len[0] * np.cos(self.theta_s),
            self.x0[1] + self.shift_len[0] * np.sin(self.theta_s),
        )
        self.pivots = [p0 for j in range(self.n)]
        for j in range(1, self.n):
            self.pivots[j] = (
                self.pivots[j - 1][0] + self.shift_len[j] * np.cos(self.theta_s),
    ...
    def Layout_Sierpinski427(self):
        ...
        for j in range(0, self.n):  # stage j
            ...
            if j == self.n - 1 and self.regType == 2:  # recalculate some variables
                ...
                temp_pivot = (
                    self.pivots[j - 1][0] + 0.9 * self.radii[j] * np.cos(temp_theta_s),
        ...
        if self.regType == 1:
            self.nodeList.append(
                (len(self.nodeList), self.pivots[self.n - 1][0], self.pivots[self.n - 1][1])
            )
...
def p_ary(n, p=2, L=None):
    """convert base 10 to base p 🔴 assume 2<=p<=10
...
def find_CC_node(str_):
    ...
    """
    if int(str_) == 0:
        return str_  # return itself if str_ is just 0
...
def make_SierpinskiGraph427(p, n, norm=False, regType=0, use_set=False):
    ...
    - regType (int):
        ...
        2: type 2 regularization (add 1 make_SierpinskiGraph427(p, n-1, use_set=use_set))
        3: type 3 regularization (add self-loop to extreme nodes)
    - use_set (bool): if True, use set ({i,j}) as item in edgeList instead of tuple

    Return
    ------
    a dict of following entries
    - A (np.array; (N,N) for unregularized): weight matrix
    - edgeList (a list of size-2 tuples (v_i,v_j))
        node index in edgeList is simply p2ten(s, p=p)
        where s is nodel p-ary string label
    - lvList (a list of hierarchy labels for edgeList): finest level is 1
    - N := p^n
    """
    N = round(p ** n)
    ...
    if regType == 0:
        ...
        for k in range(p):
            ...
            lvList.append(
                n + 1
            )  # level is n+1 (coarsest level edges) for extra p edges in regType=1 (previously -427)
    elif regType == 2:
        N_ = round(p ** (n - 1))
        GTDict = make_SierpinskiGraph427(p, n - 1, use_set=use_set)
        A = np.block(
            [[A, np.zeros((N, N_), dtype=int)], [np.zeros((N_, N), dtype=int), GTDict["A"]]]
        )
        if use_set:
            GTDict["edgeList"] = [
                {x + N, y + N} for (x, y) in GTDict["edgeList"]
            ]  # shift index by N
        else:
            GTDict["edgeList"] = [
                (x + N, y + N) for (x, y) in GTDict["edgeList"]
